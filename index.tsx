...
function UICustomizationSettings() {
  ...
  return (
    ...
    >
      <Heading tag="h3" style={{ marginBottom: 12, fontSize: 16 }}>
        UI Customization
      </Heading>
...
function HttpQServerSettings() {
  const { httpqHost, httpqPort, httpqPassword } = settings.use([
    "httpqHost",
    "httpqPort",
    "httpqPassword",
  ]);
  ...
  function getButtonColor() {
    switch (buttonState) {
      ...
      case "error":
        return Button.Colors.RED;
      default:
        return Button.Colors.BRAND;
    }
  }
  ...
  return (
    ...
    >
      <Heading tag="h3" style={{ marginBottom: 8, fontSize: 16 }}>
        HttpQ Server Configuration
      </Heading>
      ...
      <Paragraph
        style={{ marginBottom: 12, fontSize: 13, color: "var(--text-warning)" }}
      >
        ⚠️ You will need to restart Vencord for configuration changes to take
        effect
      </Paragraph>

      <Flex flexDirection="row" style={{ gap: 12, alignItems: "flex-end" }}>
        <div style={{ flex: 2 }}>
          <Heading tag="h5" style={{ fontSize: 14, marginBottom: 4 }}>
            Host
          </Heading>
          <TextInput
            value="localhost"
          ...
          <Paragraph style={{ marginTop: 4, fontSize: 12 }}>
            HttpQ communication is limited to the local machine for security
            reasons
          </Paragraph>
        </div>
        <div style={{ flex: 1 }}>
...
export default definePlugin({
  ...
  authors: [
    ...
    Devs.Av32000,
    Devs.nin0dev,
    { name: "RNDev", id: 454590946756263936n },
  ],
  settings,
  native: {
    play: "play",
